...
const renderer: RendererObject = {
  code({ text, lang }: { text: string; lang?: string | null }) {
    ...
    const isRunnable = highlightLang
      ? RUNNABLE_LANGS.has(highlightLang.toLowerCase())
      : false;
    ...
    return `<pre${preClass}>${fileLabel}<div class="code-actions">${runBtn}${copyBtn}</div><code class="hljs code-content">${highlighted}</code></pre>`;
  },
